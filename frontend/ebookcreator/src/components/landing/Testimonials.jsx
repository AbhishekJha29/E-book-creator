    ...
    <div id='testimonials' className='relative py-24 lg:py-32 bg-gradient-to-br from-violet-50 via-purple-50 to-white overflow-hidden '>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
        {TESTIMONIALS.map((testimonial, index) => (
          <div key={index} className='group relative bg-white/80 backdrop-blur-sm rounded-3xl p-8 border border-gray-100 hover:border-violet-200 hover:shadow-2xl  hover:shadow-violet-500/10 transition-all duration-300 hover:-translate-y-2'>
            ...
            <div className='flex items-center space-x-1 mb-6'>
              {[...Array(testimonial.rating)].map((_, i) => (
                <Star key={1} className='w-5 h-5 text-violet-500 fill-violet-500'/>
              ))}
          </div>
          ...
          <div className='absolute inset-0 bg-gradient-to-r from-violet-50/0 to-purple-50/0 group-hover:from-violet-50/50 group-hover:to-purple-50/30 rounded-xl transition-all duration-300 -z-10'></div>
          </div>
