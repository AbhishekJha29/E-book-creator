    ...
    return (
        <div ref={setNodeRef} style={style} className="group flex items-center bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 relative overflow-hidden">
            ...
            className={`flex-1 flex items-center p-3 text-sm rounded-l-lg text-left transition-colors ${
                ...
                ? "bg-violet-50/50 text-violet-800 font-semibold" : "text-slate-600 hover:bg-slate-100"
            }`}
...
const ChapterSidebar = ({
    ...
    return <aside className="w-80 h-full bg-white border-r border-slate-200 flex flex-col">
        ...
        <div className="p-4 border-t border-slate-200">
            <Button variant="secondary" onClick={onAddChapter} icon={<Plus />}
                className="w-full">
                New Chapter
            </Button>
        </div>
